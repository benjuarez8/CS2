...
public class ChainingHashDictionary<K, V> implements IDictionary<K, V> {
    ...
    public ChainingHashDictionary(Supplier<IDictionary<K, V>> chain) {
        this.chain = chain;
        this.buckets = new IDictionary[this.CAPACITIES[0]]; // some prime number
    }
    ...
    @Override
    public V get(K key) {
        IDictionary<K, V> dict = buckets[this.keyToIndex(key)]; // casts bucket as dictionary
    ...
    public V remove(K key) {
        ...
        V value_to_return = dict.get(key); // stores value mapped by key
...
//        Object[] temp = new Object[new_capacity];
    ...
    public V put(K key, V value) {
        ...
        IDictionary<K, V> dict = buckets[index]; // casts bucket as dictionary
    ...
    public boolean containsKey(K key) {
        ...
        return dict.containsKey(key); // returns appropriate value within bucket
    }
    ...
    public ICollection<K> keys() {
        ...
        for (IDictionary d : this.buckets) { // loops through buckets
            if (d != null) { // checks if bucket is empty
                dict = d; // creates dictionary
