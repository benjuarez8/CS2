...
public class TrieMap<A, K extends Iterable<A>, V> implements ITrieMap<A, K, V> {
    ...
    public IDeque<V> getCompletions(K prefix) {
        ...
        if (this.isPrefix(prefix)) {
            ...
            TrieMap<A, K, V> new_map = new TrieMap<>(this.collector);
            new_map.root = curr;
            for (V v : new_map.values()) {
                completions.addBack(v);
            }
        }
        return completions;
    }
    ...
    public V put(K key, V value) {
        ...
        for (A letter : key) {
            if (!curr.pointers.containsKey(letter)) {
                TrieNode<A, V> new_node = new TrieNode<>();
                curr.pointers.put(letter, new_node);
            }
            curr = curr.pointers.get(letter);
        }
        if (curr.value == null) {
    ...
    public void values(TrieNode<A, V> node, ICollection<V> values) {
        if (node == null) {
            return;
        }
        if (node.value != null) {
