...
public class NGramMap {
    ...
    public IPriorityQueue.PQElement<String>[] getCountsAfter(NGram ngram) {
        if (!this.map.containsKey(ngram)) { // returns null is map doesn't contain ngram
            return null;
        }
        IDictionary<IterableString, Integer> priorities = this.map.get(ngram); // holds "priorities"
        IPriorityQueue.PQElement<String>[] counts = new MinFourHeap.PQElement[priorities.size()]; // array to return
        IPriorityQueue.PQElement<String> count; // holds count for each string
        int i = 0; // keeps track of index of counts
        for (IterableString word : priorities.keys()) { // loops through words
            count = new IPriorityQueue.PQElement<>(word.toString(), priorities.get(word)); // count for word
            counts[i] = count; // adds to total counts
            i++; // updates index
        }
        return counts; // returns array of PQElements
    }
    ...
    public String[] getWordsAfter(NGram ngram, int k) {
        if (!this.map.containsKey(ngram)) {
            return new String[0]; // returns empty array if map doesn't contain ngram
        }
        IPriorityQueue.PQElement<String>[] counts_after = this.getCountsAfter(ngram); // holds counts_after
        TopKSort.sort(counts_after, k); // sorts appropriately
        String[] suggestions; // array of suggestions to return
        if (k < counts_after.length) { // finds appriopriate size of array
            suggestions = new String[k];
        }
        else {
            suggestions = new String[counts_after.length];
        }
        for (int i = 0; i < suggestions.length; i++) {
            suggestions[i] = counts_after[i].data; // gets appropriate suggestions from counts_after
        }
        return suggestions; // returns array of suggestions
    }
}
